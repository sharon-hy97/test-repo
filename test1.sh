...
# 清空txt
> test1.txt
username=`dialog --stdout --inputbox "你好!我想要統計大眾對於諾羅病毒的認知。現在要建立你的個人資料，請輸入你的名字(帳號):" 10 80`;
# dialog --title "你輸入的帳號" --msgbox "$username" 10 80;
passw=`dialog --stdout --passwordbox "請輸入你的密碼:" 10 80`;
gender=`dialog --stdout --title "生理性別" --radiolist "單選" 10 30 2 1 "男" on 2 "女" off`;
birth=`dialog --stdout --calendar "選擇你的生日" 15 50 9 6 1997`;
echo "帳號: $username" >> test1.txt
echo "密碼: $passw" >> test1.txt
echo "性別: $gender" >> test1.txt
echo "生日: $birth" >> test1.txt
...
#echo "回答: $an3" >> test1.txt
dialog --title "對答紀錄" --textbox ~/test1.txt 30 80
